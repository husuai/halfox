...
namespace Halfox.LongDing
{
    public class MallUtils
    {
        ...
        {
            return WebHelper.GetCookie("hasid");
        }
        ...
        {
            HttpCookie cookie = HttpContext.Current.Request.Cookies["hasid"];
            if (cookie == null)
                cookie = new HttpCookie("hasid");

            cookie.Value = sid;
            cookie.Expires = DateTime.Now.AddDays(15);
            string cookieDomain = HaConfig.MallConfig.CookieDomain;
        ...
        /// </summary>
        /// <param name="storeId">店铺id</param>
        /// <param name="storeDetails">店铺详细</param>
        /// <returns></returns>
        public static string SaveUploadStoreDetails(int storeId, HttpPostedFileBase storeDetails)
        {
            ...
            string fileName = storeDetails.FileName;
            string extension = Path.GetExtension(fileName);
            if (!ValidateHelper.IsImgFileName(fileName) || !CommonHelper.IsInArray(extension, mallConfig.UploadImgType))
                return "-2";

            int fileSize = storeDetails.ContentLength;
            if (fileSize > mallConfig.UploadImgSize)
                return "-3";

            string dirPath = IOHelper.GetMapPath(string.Format("/upload/store/{0}/details/", storeId));
            string newFileName = string.Format("sb_{0}{1}", DateTime.Now.ToString("yyMMddHHmmssfffffff"), extension);//生成文件名

            if (!Directory.Exists(dirPath))
                Directory.CreateDirectory(dirPath);
            storeDetails.SaveAs(dirPath + newFileName);
